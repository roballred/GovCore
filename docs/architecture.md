...
4. **Privilege-plane columns.** RLS alone does not protect every sensitive column: `users.instance_role` is outside the org GUC's meaning (it elevates to the operator plane). A BEFORE INSERT/UPDATE trigger (#153) rejects setting or changing `instance_role` unless the session is superuser, `BYPASSRLS` (`authDb` / `operatorDb`), or the table owner. `provisionRuntimeRole` also `REVOKE UPDATE (instance_role)` as defense in depth.

Net effect: a compromised *app* role can't cross the tenant boundary, can't self-elevate to `instance_admin`, and can't cover its tracks. The elevated privilege exists only on the privileged pools and at migration time. The honest cost — and it's a real architectural commitment, not a free toggle — is the rule that **all tenant-scoped DB access happens inside a tenant transaction.**
