 ...
 * (id, organization_id FK, scalar columns, reference + taxonomy FK columns,
 * status, timestamps, org + reference indexes, FORCEd RLS) plus a junction table
 * per `link` field. A `taxonomy` field needs `taxonomySchemaDdl` applied first.
 */
export function compileContentType(
  ...
  for (const f of def.fields) {
    if (isScalarField(f)) {
      ...
      // File under a node of the engine-owned taxonomy_nodes table. Required →
      // RESTRICT; optional → SET NULL (deleting a node unfiles the rows).
      const col = taxonomyNodeColumn(f.name)
      const onDelete = f.required ? 'RESTRICT' : 'SET NULL'
      columns.push(
        `  ${col} uuid${f.required ? ' NOT NULL' : ''} REFERENCES ${schema}.${TAXONOMY_NODES_TABLE} (id) ON DELETE ${onDelete}`,
      )
      referenceIndexes.push(
        `CREATE INDEX IF NOT EXISTS ${def.name}_${col}_idx ON ${t} (${col});`,
      )
    } else {
      throw new Error(
...
/**
 * Idempotent setup DDL for the engine-owned `taxonomy_nodes` table — the one
 * shared table every classification tree's nodes live in (org-scoped, FORCEd
 * RLS, self-referencing parent FK). Apply it once, before compiling any type
 * with a `taxonomy` field, since those FK into it.
 */
export function taxonomySchemaDdl(opts: CompileOptions = {}): string {
  const schema = opts.schema ?? DEFAULT_CONTENT_SCHEMA
  const t = `${schema}.${TAXONOMY_NODES_TABLE}`
  return (
    [
      `-- Generated by @govcore/content — the shared taxonomy classification table.`,
      `CREATE SCHEMA IF NOT EXISTS ${schema};`,
      `CREATE TABLE IF NOT EXISTS ${t} (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  organization_id uuid NOT NULL REFERENCES govcore.organizations (id) ON DELETE CASCADE,
  tree text NOT NULL,
  parent_id uuid REFERENCES ${t} (id) ON DELETE CASCADE,
  label text NOT NULL,
  slug text NOT NULL,
  sort numeric NOT NULL DEFAULT 0,
  created_at timestamptz NOT NULL DEFAULT now()
);`,
      `CREATE INDEX IF NOT EXISTS ${TAXONOMY_NODES_TABLE}_organization_id_idx ON ${t} (organization_id);`,
      `CREATE INDEX IF NOT EXISTS ${TAXONOMY_NODES_TABLE}_tree_idx ON ${t} (organization_id, tree);`,
      `CREATE UNIQUE INDEX IF NOT EXISTS ${TAXONOMY_NODES_TABLE}_slug_key ON ${t} (organization_id, tree, slug);`,
      `-- Tenant isolation by the active-org GUC, FORCEd for the non-owner runtime role.`,
      rlsBlock(t, `${TAXONOMY_NODES_TABLE}_org_isolation`),
    ].join('\n\n') + '\n'
  )
}
