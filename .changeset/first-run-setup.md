...
New package `@govcore/setup` — first-run bootstrap for an empty instance (#68). Every consumer reinvented this (GovCRM's `seed.ts`, GovEA's setup action); it's error-prone twice over, because the runtime-role grants underpin RLS and the first-admin insert predates any tenant context.
